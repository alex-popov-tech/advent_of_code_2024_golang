...
// Very suspiciously, absolutely nothing goes wrong as they begin their careful search of the vast field of helixes.
//
...
// So, a towel with the pattern ggr would have a green stripe, a green stripe, and then a red stripe, in that order.
...
// of stripe colors - that they would like to be able to display. You can use any towels you want,
// but all of the towels' stripes must exactly match the desired design.
// So, to display the design rgrgr, you could use two rg towels and then an r towel,
// an rgr towel and then a gr towel, or even a single massive rgrgr towel (assuming such towel patterns were actually available).
//
...
// In this example, the first design (brwrr) indicates that the onsen would like to be able to display a black stripe,
...
func Part1(input []byte) {
	...
	matchedDesigns := []string{}
	for _, design := range designs {
		if haveDesign(design, towels) {
			matchedDesigns = append(matchedDesigns, design)
			fmt.Println("Design", design, "is possible")
		} else {
			fmt.Println("Design", design, "is NOT possible")
		}
	}
	fmt.Printf("Possible designs are %v (%d)\n", matchedDesigns, len(matchedDesigns))
}

func haveDesign(design string, towels []string) bool {
	if design == "" {
		return true
	}

	for _, towel := range towels {
		if startsWith(design, towel) && haveDesign(design[len(towel):], towels) {
			return true
		}
	}
	return false
