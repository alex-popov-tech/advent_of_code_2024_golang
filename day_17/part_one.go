...
// Except for jump instructions, the instruction pointer increases by 2 after each instruction is processed (to move past the instruction's opcode and its operand). If the computer tries to read an opcode past the end of the program, it instead halts.
//
...
// To do this, initialize the registers to the given values, then run the given program, collecting any output produced by out instructions.
// (Always join the values produced by out instructions with commas.) After the above program halts, its final output will be 4,6,3,5,6,3,5,2,1,0.
//
...
func processInstruction(a, b, c, instructionPointer *int, instruction Instruction) (output *int) {
	...
	case 0:
		numerator := *a
		operand := expandComboOperand(instruction.operand, *a, *b, *c)
...
// Combo operand 7 is reserved and will not appear in valid programs.
// The eight instructions are as follows:
func expandComboOperand(operator, a, b, c int) int {
	switch operator {
	case 0, 1, 2, 3:
		return operator
	case 4:
...
func parse(input string) (a, b, c int, instructions []Instruction) {
	lines := strings.Split(strings.Trim(input, "\n"), "\n")
	a = atoi(lines[0][12:])
	b = atoi(lines[1][12:])
	c = atoi(lines[2][12:])
	instructionsStr := strings.Split(strings.Split(lines[4], " ")[1], ",")
	for i := 0; i < len(instructionsStr); i += 2 {
		instructions = append(instructions, Instruction{
			atoi(instructionsStr[i]),
			atoi(instructionsStr[i+1]),
		})
	}
