...
func Part1(input []byte) {
	...
	res := 0
outer:
	for _, updatePages := range updates {
		for pageIndex, page := range updatePages {
			if _, ok := rules[page]; !ok {
				break // no rules for page, skipping
			}
			for _, rule := range rules[page] {
				ruleIndex := LastIndex(updatePages, rule)
				// if rule number is in the pages list
				// and its before current page - whole update is incorrect
